...
export class DetalleArriendoComponent implements OnInit {
  ...
  cargarCalificacion() {
    this.apiCalificacion
      .getCalificacionesByIdArriendo(this.idArriendo)
      .subscribe((data) => {
        if (data.length == 0) {
        } else {
  ...
  filtrarCalificacion() {
    this.calificacion = this.calificacion.filter(
      (calificacion: any) => calificacion.idUsu == this.usuario.idUsu
    );
  ...
  onClickCalificar1() {
    ...
    this.apiCalificacion
      .putCalificaciones(this.formCalificacion.id, this.formCalificacion)
      .subscribe((data) => {
        this.recargar();
      });
  }
  ...
  agregarAFavoritos() {
    ...
    this.apiFavorito.existeFavorito(this.favorito).subscribe((data) => {
      if (data) {
        this.favorito.estado = true;
        this.favorito.id = data.id;
        this.apiFavorito
          .editarFavorito(this.favorito.id, this.favorito)
          .subscribe((data) => {
        ...
        //Nuevo favorito
        this.favorito.estado = true;
        this.apiFavorito.addFavorito(this.favorito).subscribe((data) => {
          if (data != null) {
            alert('Arriendo agregado a favoritos');
          }
        });
  ...
  setearFavorito() {
    ...
    this.apiFavorito.existeFavorito(this.favorito).subscribe((data) => {
      if (data) {
        console.log(data);
        this.favorito.id = data.id;
        this.favorito.idArr = data.idArr;
        this.favorito.idUsu = data.idUsu;
        this.favorito.estado = data.estado;
      }
    });
  }
}
